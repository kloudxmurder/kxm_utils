...
CreateThread(function()
    for item, data in pairs(ox:Items()) do
...
local function removeItem(source, inv, item, amount, meta, slot, reason)
    return ox:RemoveItem(inv, item, amount, meta, slot)
end
...
local function getItemCount(player, item)
    return ox:GetItemCount(player, item)
end
...
local function getInventoryItems(inv)
    return ox:GetInventoryItems(item)
end
